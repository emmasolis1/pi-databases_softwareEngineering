...
import { SpecificReportDetails } from '../components/specificReportEmployee/report-specific-information';
...
class SpecificReport extends React.Component {
  constructor(props) {
    ...
    this.state = {
      report: [],
      isLoaded: false,
      APIUrl: URL + 'employeeReport',
    };
  }

  componentDidMount() {
    var my_url = this.state.APIUrl + '?employeeID=' + sessionStorage.getItem('userID');
    my_url = my_url + '&employerID=' + sessionStorage.getItem('employerIDReportToVisualize') + '&projectName=' + sessionStorage.getItem('projectNameReportToVisualize') + '&paymentDate=' + sessionStorage.getItem('paymentDateReportToVisualize');
